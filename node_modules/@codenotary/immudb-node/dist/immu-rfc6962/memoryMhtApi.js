...
export function createFromValues(values) {
    if (values.length < 1) {
        throw new RangeError('values must be non empty');
    }
    function allocateTree(numberOfLeafs) {
        const tree = [];
    ...
    const tree = allocateTree(values.length);
    ...
    while (currentWidth > 1) {
        ...
        while (currentNodeIndex + 1 < currentWidth) {
            tree[currentLevel + 1][parentNodeIndex] = hash.ofBuffers(Buffer.of(0x01), tree[currentLevel][currentNodeIndex], tree[currentLevel][currentNodeIndex + 1]);
...
export function getRoot(treeData) {
    return treeData[treeData.length - 1][0];
}
export function getWidth(treeData) {
    return treeData[0].length;
}
export function getHight(treeData) {
    return treeData.length;
}
export function getLevelWidht(treeData, levelIndex) {
...
export function getLeaf(treeData, index) {
...
export function getNode(treeData, levelIndex, index) {
...
export function getParentNodeIndex(treeData, levelIndex, index) {
    const levelWidth = getLevelWidht(treeData, levelIndex);
    if (index < 0
        || index > (levelWidth - 1)) {
        throw new RangeError('index must be greater or equall 0 and lesser than level width');
...
export function generateInclusionProofDataFor(props) {
    const { leafIndex: leafIndex, usingMerkleTree: tree, } = props;
...
function powerOfTwoLessOrEqThan(uInt32) {
    uInt32 |= uInt32 >> 1;
    uInt32 |= uInt32 >> 2;
    uInt32 |= uInt32 >> 4;
    uInt32 |= uInt32 >> 8;
...
//# sourceMappingURL=memoryMhtApi.js.map