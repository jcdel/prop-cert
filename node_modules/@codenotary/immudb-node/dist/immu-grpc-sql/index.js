export function grpcSqlObjectNamedValueToNamedValues(objectNamedValue) {
    return Object.entries(objectNamedValue).map(([name, grpcSqlValue]) => ({
        name,
        ...grpcSqlValueToSqlValue(grpcSqlValue)
    }));
}
export function grpcQueryResultToListoOfSqlNamedValues(queryResult) {
    return queryResult.rows.map(grpcSqlRowToSqlNamedValues);
}
export function grpcSqlRowToSqlNamedValues(grpcRow) {
    return grpcRow.values.map((val, i) => ({
        name: grpcRow.columns[i],
        ...grpcSqlValueToSqlValue(val)
    }));
}
export function sqlNamedValueToGrpcSqlNamedParam(param) {
    switch (param.type) {
        case 'BOOLEAN':
...
export function sqlValueToGrpcSqlValue(param) {
    switch (param.type) {
        case 'BOOLEAN':
...
export function grpcSqlNamedParamToSqlNamedValue(param) {
...
export function grpcSqlValueToSqlValue(param) {
    switch (param.value) {
        case 'b':
