...
export function createGenerator() {
    return function computeHash(data) {
 ...
 *
 */
export function fromBuffers(buffers) {
    const hashState = createHash('sha256');
 ...
 * ```
 */
export function fromTreeBuffers(buffers) {
    let hashState = createHash('sha256');
    fromTreeBuffersInternal(hashState, buffers);
    return hashState.digest();
}
function fromTreeBuffersInternal(inout__hashState, buffers) {
    for (const buffer of buffers) {
...
export function ofTreeBuffers(...buffers) {
 ...
 *
 */
export function ofBuffers(...buffers) {
 ...
 *
 */
export function fromIterableBuffers(buffers) {
    return stream.pipeline(stream.Readable.from(buffers), createHash('sha256')).digest();
}
